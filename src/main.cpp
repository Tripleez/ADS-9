...
void runExperiment(int max_n) {
    ...
    for (int n = 1; n <= max_n; ++n) {
        ...
        auto perm1_time = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
        
        start = std::chrono::high_resolution_clock::now();
        auto perm2 = tree.getPerm2(random_perm_num);
        end = std::chrono::high_resolution_clock::now();
        auto perm2_time = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
